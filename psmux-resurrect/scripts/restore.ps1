...
try {
    ...
    for ($si = 0; $si -lt $totalSessions; $si++) {
        ...
        if ($reuseExisting) {
            # Reuse existing session: rename its (only) window to the saved name
            # and capture that window's pane id to target.
            if ($firstWindow.name) {
                & $PSMUX rename-window -t $sessionName $firstWindow.name 2>&1 | Out-Null
            }
            $firstPaneId = Get-PaneId (& $PSMUX display-message -t $sessionName -p '#{pane_id}' 2>&1)
        } else {
